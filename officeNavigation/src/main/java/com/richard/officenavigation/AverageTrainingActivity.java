...
public class AverageTrainingActivity extends BaseActivity implements
    ...
    private void handleTrainingStart() {
        if (mTrainingStage < 4) {
            ...
            if (time.equals("")) {
                m(getString(R.string.train_time));
            } else {
                int sec = Integer.parseInt(time);
                if (sec > 1) {
                    mDlgCompass.setTitle(getString(R.string.sure_direction)
                            + getString(mOrients[mTrainingStage]) + getString(R.string.direction));
                    mDlgCompass.show();
                } else {
                    m(getString(R.string.train_time_should_bigger_than_ten_sec));
                }
            }
        } else if (mTrainingStage == 4) {
    ...
    private void handleActionDone() {
        InfoDialog.newInstance(this, getString(R.string.train_done), getString(R.string.is_store_orgin_data),
    ...
    public void onConfirmOrientation(int orientation) {
        if (orientation != mTrainingStage) {
            m(getString(R.string.change_train_direction));
        } else {
    ...
    public boolean onLongClick(View v) {
        ...
        mBtnTrain.setOnClickListener(new OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                InfoDialog.newInstance(AverageTrainingActivity.this, getString(R.string.train_done),
                        getString(R.string.is_store_orgin_data), new DialogInterface.OnClickListener() {
